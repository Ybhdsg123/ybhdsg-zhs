      ...
      >
        ...
        <image
          :class="[
            ...
            {
              'img-hide':
                item2[hideImageKey] == true || item2[hideImageKey] == 1,
            },
...
export default {
  ...
  computed: {
    ...
    m() {
      const column_margin = `${
        (100 -
          (100 / this.data.column - +this.data.columnSpace).toFixed(5) *
            this.data.column) /
        (this.data.column - 1)
      }%`;
      ...
      for (let i = 0; i < this.data.list.length; i++) {
        ...
        for (let j = 0; j < cv.length; j++) {
          if (newValue[i] && i === cv[j].index) {
            this.data[`column_${this.data.list[i].column}_values`][j] =
              Object.assign(cv[j], newValue[i]);
            this.msg++;
            break;
          }
        }
