<template>
    <view class="ellpsis-text">
        <view class="text-content" :style="{ '-webkit-line-clamp': showText ? Infinity : lines }">
            <text v-if="prefixText" class="fs32 fw700 fc0">{{ prefixText }} </text>
            <!-- 展示文本 -->
            <text> {{ content }}</text>
            <!-- 站位文本 -->
        ...
        <view v-if="isEllipsis" class="btn-text" @click="shiftText">
            ...
            <view v-else>
                展开 <text class="iconfont icon-xiala"></text>
            </view>
        </view>
    </view>
</template>
